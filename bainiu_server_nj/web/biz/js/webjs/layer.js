...
//        return jsPath.substring(0, jsPath.("/") + 1);
//	
		...
		return localhostPaht+"biz/js/webjs/";
    }(),
...
layer.close = function(index){
    var layero = $('#'+ doms[0] + index), type = layero.attr('type');
    if(!layero[0]){
    	var layero = $(window.parent.document.getElementById(doms[0] + index)), type = layero.attr('type');
    }
    if(!layero[0]) return;
    if(type === ready.type[1] && layero.attr('conType') === 'object'){
        ...
        if(type === ready.type[2]){
            try {
                ...
                if(!iframe){
                	iframe = $(window.parent.document.getElementById(doms[4]+index))[0];
                	$(window.parent.document.getElementById('layui-layer-moves')).remove();
    				$(window.parent.document.getElementById('layui-layer-shade'+ index)).remove();   				
    				$(window.parent.document.getElementById('layui-layer'+ index)).remove();	//---?
                	ready.rescollbar(index);
                	$(window.parent.document).off('keydown', ready.enter);
                }
    ...
    if(! $('#layui-layer-moves, #layui-layer-shade' + index)){
    	$(window.parent.document.getElementById('layui-layer-moves')).remove();
    	$(window.parent.document.getElementById('layui-layer-shade'+ index)).remove();
    }else{
